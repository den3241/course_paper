...
numgen = 4 # Количество генов
uzel = 23 # Количество узлов
numchrom = 400000
...
def fit(num): # Фитнес-функция
    ...
    summ = 0
    for i in range(numpoints):
        summ = summ + (afact * x[i]**3 + bfact * x[i]**2 + cfact * x[i] + dfact - y[i])**2
    return 10000 - summ
    ...
    for i in range (numchrom): # МУТАЦИЯ Пробегаемся по всемм хромосомам
        for j in range(numgen): # Проегаемся по всем генам
            if randint(0, 100) < mut: # Если вероятность мутации больше рандомного числа то происходит мутация
                ...
                if mingen[j] < (pop[i][j] + m) < maxgen[j]:
                    pop[i][j] += m # Произошла мутация в текущей хромосоме
...
print('a =', round(abestfact, 1), 'b =', round(bbestfact, 1), 'c =', round(cbestfact, 1), 'd =', round(dbestfact, 1))
print()
print() # Пропуск строки
print(f"Время работы программы (секунд): {int(time()-st)}") # Вывод времени работы программы

fig, ax=plt.subplots()
y1 = lambda x: abestfact * x**3 + bbestfact * x**2 + cbestfact * x + dbestfact

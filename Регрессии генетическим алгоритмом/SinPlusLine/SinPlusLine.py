...
from numpy import zeros, sin # Библиотека для заполнения массивов
...
for i in range(numpoints):
    y.append(100.5 * sin(0.7 * x[i] + 6.5) + 100.5 * x[i] + 20.5)
amax = 300
bmax = 50
cmax = 50
dmax = 300
...
numgen = 5 # Количество генов
uzel = 9 # Количество узлов
numchrom = 300000
minnumchrom = (uzel + 1)**numgen # Количество хромосом
print("Минимальное кол-во хромосом:", minnumchrom)
...
def fit(num): # Фитнес-функция
    ...
    dfact = (dmax - dmin) * d / dg[3] + dmin
    efact = (emax - emin) * e / dg[4] + emin
    """ ОПРЕДЕЛЕНИЕ ФИТНЕС-ФУНКЦИИ (ГЕНЫ НАХОДЯТСЯ В МАССИВЕ p) """
    summ = 0
    for i in range(numpoints):
        summ = summ + (afact * sin(bfact * x[i] + cfact) + dfact * x[i] + efact - y[i])**2
    return 10000 - summ
...
for z in range(e): # Пробег по эпохам для поиска лучшего значения
    ...
    for i in range (numchrom): # МУТАЦИЯ Пробегаемся по всемм хромосомам
        for j in range(numgen): # Проегаемся по всем генам
            if randint(0, 100) < mut: # Если вероятность мутации больше рандомного числа то происходит мутация
...
print(f" - Фитнес-функция: {maxpopp}")
print(f' - Хромосома: ', end='')
abest = pop[indexmax][0]
bbest = pop[indexmax][1]
cbest = pop[indexmax][2]
dbest = pop[indexmax][3]
ebest = pop[indexmax][4]
abestfact = (amax - amin) * abest / dg[0] + amin
bbestfact = (bmax - bmin) * bbest / dg[1] + bmin
cbestfact = (cmax - cmin) * cbest / dg[2] + cmin
dbestfact = (dmax - dmin) * dbest / dg[3] + dmin
ebestfact = (emax - emin) * ebest / dg[4] + emin
print('a =', round(abestfact, 1), 'b =', round(bbestfact, 1), 'c =', round(cbestfact, 1), 'd =', round(dbestfact, 1), 'e =', round(ebestfact, 1))
print()
print() # Пропуск строки
print(f"Время работы программы (секунд): {int(time()-st)}") # Вывод времени работы программы

fig, ax=plt.subplots()
y1 = lambda x: abestfact * sin(bbestfact * x + cbestfact) + dbestfact * x + ebestfact
x1 = numpy.linspace(0, 10, 1000)
plt.plot(x1, y1(x1))
